...
export default function ProductPageAdminControls({ productId }: { productId: string }) {
  ...
  const handleDelete = async () => {
    ...
    const res = await fetch(`${API_URL}/api/products/id/${productId}`, { method: 'DELETE', credentials: 'include' });
    if (res.ok) { router.push('/gallery'); router.refresh(); }
  };

  return (
    <div className="flex gap-4 text-xs text-stone-400 border-b border-stone-100 pb-4">
